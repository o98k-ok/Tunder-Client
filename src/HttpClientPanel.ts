...
export class HttpClientPanel {
    ...
    <head>
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src ${webview.cspSource} 'unsafe-inline' https://cdn.jsdelivr.net; connect-src https://cdn.jsdelivr.net;">
        ...
        <style>
            ...
            #requestBodyEditor {
                height: 600px;
                --jse-background-color: var(--vscode-editor-background);
                --jse-key-color:var(--json-key-color);
                --jse-value-color: var(--json-string-color);
                --jse-value-color-number: var(--json-number-color);
                --jse-value-color-boolean: var(--json-boolean-color);
                --jse-value-color-null: var(--json-null-color);
                --jse-value-color-string: var(--json-string-color);
                --jse-value-color-url: var(--json-string-color);
                --jse-delimiter-color: var(--vscode-editor-foreground);
                --jse-panel-background: var(--vscode-editor-background);
                --jse-panel-color: var(--vscode-editor-foreground);
            }
        </style>
    </head>
    <body>
            ...
            (function() {
                ...
                function initJsonEditor() {
                    editor = createJSONEditor({
                        target: document.getElementById('requestBodyEditor'),
                        props: {
                            content: {
                                text: '',
                            },
                    ...
                    // 手动为编辑器元素添加事件监听
                    const editorElement = document.getElementById('requestBodyEditor');
                    if (editorElement) {
                ...
                // 立即初始化编辑器，不等待DOMContentLoaded事件
                initJsonEditor();
                ...
                function getRequestData() {
                    ...
                    console.log('获取请求数据:', { method, url, headers, body });
                ...
                function updateRequestData(data) {
                    ...
                    console.log('更新请求数据:', { method, url, headers, body });
                ...
                function saveCurrentRequest() {
                    ...
                    console.log('保存请求，数据:', requestData);
                    vscode.postMessage({
                        command: 'saveRequest',
                        data: requestData
                    });
